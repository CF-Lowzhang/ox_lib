            ...
            if next(insideZones) then
                tick = SetInterval(function()
                    for _, zone in pairs(insideZones) do
                        ...
                        local contains = zone:contains(GetEntityCoords(cache.ped), true)
                        if not contains then  
                            zone.insideZone = false
                            insideZones[zone.id] = nil
                            if zone.onExit then
...
function lib.zones.DeleteZones(pid) 
    for _ , Zone in pairs(Zones) do  
        if Zone.pid == pid then 
            local ZID = Zone.id 
            Zones[ZID] = nil
            lib.grid.removeEntry(Zone)
            if lib.contxt == 'server' then return end
            insideZones[ZID] = nil
            table.remove(exitingZones, exitingZones:indexOf(Zone))
            table.remove(enteringZones, enteringZones:indexOf(Zone))
            return true
...
function lib.zones.ExistZones(pid) 
    for _ , Zone in pairs(Zones) do  
        if Zone.pid == pid then 
            return true
        end
